...
{
    vector <vector <char> > matrixMake;
    
    matrixMake.resize(rows);

    for(int row = 0; row < matrixMake.size(); row++)
    {
            matrixMake[row].resize(columns);
            for(int col = 0; col < matrixMake[row].size(); col++)
            {
                    matrixMake[row][col] = value;
            }
    }
